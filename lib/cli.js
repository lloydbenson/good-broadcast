...
exports.run = function (options) {
    ...
    Hoek.assert(configPath, '-c option must be a valid file path');
    ...
    // Path implies resume
    config.resume = config.resume || !!(config.resumePath && config.resumePath.length);
    // Either use a default or the path supplied
    config.resumePath = config.resumePath || Path.join(Path.dirname(config.log), '.lastindex');
