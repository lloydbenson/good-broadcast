...
internals.logLastIndex = function (index, done) {
    ...
    var lastIndexStream = Fs.createWriteStream(internals.lastIndexPath, { flags: 'a' });
    lastIndexStream.on('error', function (err) {
        console.error(err);
        lastIndexStream.removeAllListeners();
    });

    //var fd = Fs.openSync(internals.lastIndexPath, { flags: 'a'});
    //Fs.ftruncateSync(fd, 0);
    //Fs.writeSync(fd, index)


    Fs.truncate(internals.lastIndexPath, function (error) {

        // If there isn't an error or if the error is no file, write to it
        if (!error || error && error.errno === 34) {
            lastIndexStream.write(index + '', function (err) {
        ...
        return function (callback) {
            ...
            getStats(config.path, function(error, result) {
                ...
                log.get(config.path, startIndex, function (bytesRead, logResult) {
                    ...
                    internals.logLastIndex(startIndex, function () {
